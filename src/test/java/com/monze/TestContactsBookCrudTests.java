...
public class TestContactsBookCrudTests {
    ...
    //  @Test
    //  public void testContactSearchByName() throws SQLException {
    //      // When there is a contacts saved as LEON
    //      String expectedContacts = "LEON";
    //      //int expectedContacts1 = 1;
    ...
    //      String sql3 = "INSERT INTO tblphone(pname,pnumber1,pnumber2) VALUES('LEON',8888,21);";
    //   //   when(connection.prepareStatement(sql1)).thenReturn(preparedStatement);
    //   //   when(preparedStatement.executeUpdate(sql1));
    //    //  when(connection.prepareStatement(sql2)).thenReturn(preparedStatement);
    //   //   when(preparedStatement.executeUpdate(sql2));
    //      when(connection.prepareStatement(sql3)).thenReturn(preparedStatement);   
    //      when(preparedStatement.executeUpdate(sql3));

    //      when(preparedStatement.executeQuery()).thenReturn(resultSet);
    //      when(resultSet.next()).thenReturn(true);
    //      when(resultSet.getString(2)).thenReturn(expectedContacts);
    //     // when(resultSet.getInt(1)).thenReturn(expectedContacts);

    // //     /// Tests
    //      assertEquals(expectedContacts, contactsDatabase.findData("LEON").getString(2));
    //      verify(resultSet).getString(2);
    //      verify(resultSet).getInt(1);
    // //     // When there are not any contacts
    //      when(resultSet.next()).thenReturn(false);
    //      assertEquals(expectedContacts, contactsDatabase.findData("LEONx").getString(2));
    //  }
